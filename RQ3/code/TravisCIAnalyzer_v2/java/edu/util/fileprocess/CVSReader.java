...
public class CVSReader {
	...
	public List<TravisCommits> loadTravisCommits(String filepath) throws IOException {
		List<TravisCommits> evalprojects = new ArrayList<>();
		try (Reader reader = Files.newBufferedReader(Paths.get(filepath));
				CSVParser csvParser = new CSVParser(reader,
						CSVFormat.DEFAULT.withFirstRecordAsHeader().withIgnoreHeaderCase().withTrim());) {
			for (CSVRecord csvRecord : csvParser) {
				// Accessing values by Header names
				String id = csvRecord.get("CommitID");
				String message = csvRecord.get("CommitMessage");
				String name = csvRecord.get("GitAuthor");
				String files = csvRecord.get("Lsof ModifiedFiles");
				String repo =csvRecord.get("ProjectName");
				//String problemBuildState=csvRecord.get("problemBuildState");

				TravisCommits commitinfo = new TravisCommits(repo, id, message, name, files);
				//commitinfo.setMainProblem(mainProblem);
				//commitinfo.setAllProblems(allProblems);
				//commitinfo.setProblemBuildState(problemBuildState);
