...
# normalize categories by .lower()
df['GHACategory'] = df['GHACategory'].str.lower()
df['GHACategory_2'] = df['GHACategory_2'].str.lower()
...
#
# # exit()
# # add a column 'final_call' to the dataframe, which specifies if the values of GHACategory if it equals the value of GHACategory_2, empty if not
# df['final_call_gha_category'] = df.apply(lambda x: x['GHACategory'] if x['GHACategory'] == x['GHACategory_2'] else '', axis=1)
#
# # drop GhaCategory_2 column and GHACategory column
# # df = df.drop(['GHACategory', 'GHACategory_2'], axis=1)
#
# # save the dataframe to a new excel file
# df.to_excel('..\\data\\conflict_resolution_rq1.xlsx', index=False)
#
