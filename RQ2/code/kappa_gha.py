...
# print the kappa scores for each category
for i, category in enumerate(categories):
    print(f"Kappa score for {category}: {kappa_scores[i]}")

# add column final_call_coevolution_categories. this column specifies if the values of Categories_rater_1 if it contains the same categories as Categories_rater_2, empty if not
# use the binarized labels to check for equality, use the row index to get the corresponding binarized labels
merged_df['final_call_coevolution_categories'] = merged_df.apply(lambda x: x['Categories_rater_1'] if rater1_bin[x.name] == rater2_bin[x.name] else '', axis=1)

# drop Categories_rater_1 and Categories_rater_2 columns
# merged_df = merged_df.drop(['Categories_rater_1', 'Categories_rater_2'], axis=1)

# save the dataframe to a new excel file
output_path = "..\\data\\RQ2_coevolution_taxonomy_gha_kappa_scores_conflict_resolution.xlsx"
