...
file_path = "..\\data\\RQ2_coevolution_taxonomy_travis.xlsx"
...
rater_1_list = [x.replace('[','').replace(']','').replace('"','').replace("'",'') for x in rater_1_list]
rater_2_list = [x.replace('[','').replace(']','').replace('"','').replace("'",'') for x in rater_2_list]



#convert all values of lists to string
rater_1_list = [str(x) for x in rater_1_list]
rater_1_list = [x.lower() for x in rater_1_list]
rater_2_list = [str(x) for x in rater_2_list]
rater_2_list = [x.lower() for x in rater_2_list]

# extract unique categories from both raters, ignoring spaces
rater_1_categories = set()
rater_2_categories = set()

for categories_temp in rater_1_list:
    cats_temp_array = categories_temp.split(',')
    for cat in cats_temp_array:
        rater_1_categories.add(cat.strip())

for categories_temp in rater_2_list:
    cats_temp_array = categories_temp.split(',')
    for cat in cats_temp_array:
        rater_2_categories.add(cat.strip())


# find categories only in rater and not in categories
rater_1_categories = rater_1_categories - set(categories)
rater_2_categories = rater_2_categories - set(categories)

# replace data and model versioning with model and data versioning in rater_1 and rater_2 lists
rater_1_list = [x.replace('data and model versioning','model and data versioning') for x in rater_1_list]
rater_2_list = [x.replace('data and model versioning','model and data versioning') for x in rater_2_list]
...
# print different categories and corrsponding kappa scores
for i in range(len(categories)):
    print(f'Kappa Score for {categories[i]}: {kappa_scores[i]}')
...
output_path = "..\\data\\RQ2_coevolution_taxonomy_travis_kappa_scores.xlsx"
